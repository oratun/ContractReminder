...
from .forms import LoginForm, RegistrationForm, ChangePasswordForm,\
    PasswordResetRequestForm, PasswordResetForm, ChangeEmailForm, CopyEmailForm
...
def resend_confirmation():
    ...
    send_email(current_user.email, '验证你的邮箱',
               'auth/email/confirm', user=current_user, token=token)
    flash('一封新的确认邮箱已发至你的邮箱。')
    return redirect(url_for('main.index'))
...
@auth.route('/copy-email', methods=['GET', 'POST'])
@login_required
def copy_email():
    form = CopyEmailForm()
    if form.validate_on_submit():
